...
const DEFAULT_OPENAI_IMAGE_ATTEMPTS = 1
...
function buildPairsPrompt(language, pairCount, topic) {
  return [
    ...
    '',
    'Requirements:',
    '- Use REAL dictionary words only. No invented words, no misspellings, no rare/archaic forms.',
    '- Words must be concrete, child-friendly singular nouns and should rhyme clearly in the target language.',
    '- Keep words short when possible and avoid offensive or abstract vocabulary.',
    '- Prefer high-confidence, perfect rhymes over creative near-rhymes.',
    '- Avoid verb forms, adjective forms, plurals, and awkward inflections unless they are standard noun lemmas.',
    '- For each pair, both words must share the same final stressed vowel sound and ending consonant pattern.',
    '- Avoid duplicates across all words in the worksheet.',
    '- If theme words do not rhyme well, choose everyday nouns outside the theme instead of weak rhymes.',
...
function buildAlternativeWordPrompt({ currentWord, pairedWord, language, topic }) {
  return [
    ...
    '{"word":"new noun"}',
    'Rules:',
    '- Use a REAL dictionary word only.',
    '- One concrete, child-friendly singular noun.',
    '- Must be a strong/perfect rhyme with the paired word in the target language.',
    '- Keep it short and easy to draw.',
    '- No punctuation-only output.',
    '- No verbs/adjectives/plural forms unless they are standard noun lemmas.',
...
export default async function handler(req, res) {
  ...
  const imageAttempts = DEFAULT_OPENAI_IMAGE_ATTEMPTS
  const imageConcurrency = DEFAULT_OPENAI_IMAGE_CONCURRENCY
  ...
  try {
    ...
    return res.status(200).json({
      ...
      usedTextModel: usedModel,
      usedImageModels: [openAiImageModel],
      imageWarning:
